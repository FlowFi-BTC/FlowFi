      ...
      <div className="mx-auto space-y-6">
        ...
        <div className="neo-border-thick bg-white  p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="w-full sm:w-auto">
            <input
              ...
              className="w-full sm:w-64  neo-border bg-[#f7f7f7] px-4 py-2 text-xs font-medium text-black focus:outline-none focus:ring-2 focus:ring-[#a8ff3e]"
            />
          </div>
        </div>
        ...
        {isInvestor && myFundings.length > 0 && (
          <div className="neo-border-thick bg-white  p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] space-y-4">
            <h2 className="font-syne text-lg font-medium text-black">My Active sBTC Holdings</h2>
            <div className="space-y-3">
              {myFundings.map((item) => (
                <div
                  key={item.id}
                  className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3  neo-border bg-[#c4b5fd]/20 p-4 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]"
                >
                  <div>
                    <span className="font-syne font-medium text-black text-sm block">{item.businessName}</span>
                    <span className="text-xs font-medium text-gray-600">
                      Amount: <span className="text-[#6B46C1] font-medium">{item.amountSbtc} sBTC</span> • Due: {new Date(item.dueDate).toLocaleDateString()}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className=" neo-border bg-[#a8ff3e] px-3 py-1 text-xs font-medium text-black">
                      {item.status}
                    </span>
                    <Link
                      to={`/receivable/${item.receivableId}`}
                      className=" neo-border bg-white px-3 py-1 text-xs font-medium text-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5"
                    >
                      View Details →
                    </Link>
                  </div>
        ...
        {/* Marketplace Grid */}
        <div className="neo-border-thick bg-white  p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] space-y-4">
          <h2 className="font-syne text-lg font-medium text-black">
            Explore Open Receivables 
          </h2>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {marketplaceItems.map((item) => (
                <div
                  key={item.id}
                  className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3  neo-border bg-[#f7f7f7] p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-white transition-all"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-syne font-medium text-black text-base">{item.title}</span>
                      <span className=" neo-border bg-[#a8ff3e] px-2.5 py-0.5 text-[10px] font-medium text-black">
                        {item.verificationStatus}
                      </span>
                    </div>
                    <div className="text-xs font-medium text-gray-600">
                      Issuer: <span className="text-black">{item.businessName}</span> • Valuation:{' '}
                      <span className="text-[#6B46C1] font-medium">${item.amountUsd} USD</span>
                    </div>
                  </div>

                  <Link
                    to={`/receivable/${item.id}`}
                    className=" neo-border bg-[#22d3ee] px-5 py-2.5 text-xs font-medium text-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform"
                  >
                    View & Fund sBTC →
                  </Link>
                </div>
